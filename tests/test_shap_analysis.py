...
def test_get_smiles_true_predicted(smiles_order, true_ys, preds, task, classes_order):
    ...
    for i in range(len(alternate)):
        ...
        if task == Task.REGRESSION:
            assert smiles_true_predicted_df.loc[al_smi].predicted == al_pred, print(
                smiles_true_predicted_df.loc[al_smi].predicted - al_pred)
        elif task == Task.CLASSIFICATION:
            class_columns = [Stability(i).name for i in classes_order]
            assert all(smiles_true_predicted_df.loc[al_smi, class_columns] == al_pred), print(
                smiles_true_predicted_df.loc[al_smi].predicted - al_pred)
        else:
...
def test_get_present_features(x_train, threshold):
    ...
    for i in range(x_train.shape[1]):
        ...
        if (non_zero_elements / x_train.shape[0] >= threshold) and (
                non_zero_elements / x_train.shape[0] <= 1 - threshold):
            features.append(i)
...
def test_filter_samples(my_smis, my_feats, X_full, X_full_df, shap_values, task, smiles_order):
    to_analyse_X, to_analyse_df, to_analyse_shaps, smi_order, mol_indices, feature_order = filter_samples(my_smis,
                                                                                                          my_feats,
                                                                                                          X_full,
                                                                                                          shap_values,
                                                                                                          task,
    ...
    for new_feat, old_feat in enumerate(feature_order):
        for new_mol, old_mol in enumerate(mol_indices):
            assert X_full[old_mol, old_feat] == to_analyse_X[
                new_mol, new_feat], f"{old_mol}-{old_feat}  {new_mol}-{new_feat}"
...
def test_find_separation_point_analyses(shap_values, X_full, feature_order, task):
    ...
    for class_idx in classes:
        for i in feature_order:
            ...
            assert len(best_thresholds) == len(kbest_thresholds), AssertionError(
                f'{best_thresholds} != {kbest_thresholds}')
            assert set(best_thresholds) == set(kbest_thresholds), AssertionError(
                f'{best_thresholds} != {kbest_thresholds}')
    ...
    for f in feature_order:
        ...
        for my_results, ref, c in zip(separation_results, reference_result, classes):
            ...
            for my in my_results:
                l0, l1, r0, r1 = situation_at_threshold(my.thresholds, f, shap_values, X_full, feature_order, task,
                                                        class_index=c, print_func=None)
...
def all_tests(some_model, some_shaps):
    ...
    shap_cfg, smiles_order, X_full, morgan_repr, true_ys, preds, classes_order, expected_values, shap_values, background_data = load_shap_files(
        some_shaps, task)
    ...
    to_analyse_X, to_analyse_df, to_analyse_shaps, smi_order, mol_indices, feature_order = filter_samples(smiles_order,
                                                                                                          my_feats,
                                                                                                          X_full,
                                                                                                          shap_values,
                                                                                                          task,
