...
def parse_shap_config(config_path):
    ...
    try:
        config[utils_section]["unlog"] = str_to_bool(config[utils_section]["unlog"])
    except KeyError:
        config[utils_section]["unlog"] = False
    return config
...
def parse_representation_config(config_path):
    config = read_config(config_path)
    if 'morgan_nBits' in config[utils_section]:
        config[utils_section]['morgan_nbits'] = config[utils_section]['morgan_nBits']
        del(config[utils_section]['morgan_nBits'])
...
def parse_task_config(config_path):
    ...
    try:
        ...
        print("TPOT NOT FOUND. Task config has strings instead.")

    if 'cutoffs' in config[utils_section]:
        if config[utils_section]['cutoffs'] == 'metstabon':
            import metstab_pred.src.data as data
