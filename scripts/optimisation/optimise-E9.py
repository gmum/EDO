...
from metstab_pred.src.optimisation.utils import find_experiment, load_train_test, load_predictions, load_shap_files, \
    load_model
from metstab_pred.src.optimisation.utils import filter_correct_predictions_only, group_samples, intersection_list, difference_list
...
from metstab_pred.src.optimisation.evaluation import rule_stats, optimisation_stats, evaluate_optimisation, \
    evaluate_history

"""
Obecnie optymalizujemy tylko poprawnie przewidziane zwiazki.
A co by bylo, gdybysmy optymalizowali niepoprawnie przewidziane zwiazki?
Oceniane: osobno correctly predicted zwiazki, osobno incorrectly predicted zwiazki.
"""
...
if __name__ == "__main__":
    ...
    group_scores = {}
    for group_tr, group_te, group_name in zip(groups_train_samples, groups_test_samples, group_names):
        logger_wrapper.logger.info(group_name)
        ...
        rules_df.to_csv(osp.join(saving_dir, f'{timestamp}-{group_name}-rules.csv'))
        ...
        for c_smis, c_name in ((test_correct_smi, 'correctly_predicted'),
            ...
            logger_wrapper.logger.info(f'Optimisation of {c_name} compounds')
            ...
            shapator_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_name}-{c_name}-shapator.csv'))
            ...
            independator_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_name}-{c_name}-independator.csv'))
            ...
            regressor_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_name}-{c_name}-regressor-logged.csv'))
            ...
            reg_unlogged_df.to_csv(
                osp.join(saving_dir, f'{timestamp}-history-{group_name}-{c_name}-regressor-unlogged.csv'))
            ...
            group_scores[f'{group_name}_{c_name}_scores'] = group_results
