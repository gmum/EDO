...
from edo.optimisation.utils import get_correct_predictions, group_samples, intersection_list
...
Ocenianie: osobno kazda grupa zwiazkow.
"""
...
if __name__ == "__main__":
    ...
    # # # # # # # # # # #
    group_scores = {}
    logger_wrapper.logger.info("One set of rules")
    group_tr_name = 'all'
    ...
    for group_te, group_te_name in zip(groups_test_samples, group_names):
        ...
        opis_exp = optimisation_stats(present_features, my_rules, my_samples, samples_for_evaluation, print_func=pprint)
        ...
        shapator_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_te_name}-shapator.csv'))
        ...
        independator_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_te_name}-independator.csv'))
        ...
        regressor_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_te_name}-regressor-logged.csv'))
        ...
        reg_unlogged_df.to_csv(osp.join(saving_dir, f'{timestamp}-history-{group_te_name}-regressor-unlogged.csv'))
        ...
        group_scores[f'{group_te_name}_scores'] = group_results
