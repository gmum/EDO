...
#SBATCH --output="E2S2-WS.out"
#SBATCH --error="E2S2-WS.err"
...
do
  ...
  do
    ...
    do
      ...
      do
        singularity run --pwd $HOME/projects/metstab_pred -B $HOME:$HOME,$SHRD:$SHRD $SHRD/metpred3.simg python -u scripts/optimisation/optimise.py ${DATA} ${SPLIT} ${FP} ${TASK} ${M1} ${M2} ${MR} $at_once $n_times $ws_min ${HI_GAMMA} ${HI_METRIC} ${HI_MIN} ${UNIMP_MIU} ${UNIMP_METRIC} ${UNIMP_MAX} $seed ${RESULTS_DIR} optimisation_results/exp2_ws_hi_params/s2_ws_optimise --skip_criterion_check
        singularity run --pwd $HOME/projects/metstab_pred -B $HOME:$HOME,$SHRD:$SHRD $SHRD/metpred3.simg python -u scripts/optimisation/optimise.py ${DATA} ${SPLIT} ${FP} ${TASK} ${M1} ${M2} ${MR} $at_once $n_times $ws_min ${HI_GAMMA} ${HI_METRIC} ${HI_MIN} ${UNIMP_MIU} ${UNIMP_METRIC} ${UNIMP_MAX} $seed ${RESULTS_DIR} optimisation_results/exp2_ws_hi_params/s2_ws_baseline --baseline --skip_criterion_check
      done
    done
  done
done
