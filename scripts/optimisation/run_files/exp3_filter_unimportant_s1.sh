...
#SBATCH --output="E3S1.out"
#SBATCH --error="E3S1.err"
  ...
  do
    ...
    do
      ...
      do
        ...
        do
          singularity run --pwd $HOME/projects/metstab_pred -B $HOME:$HOME,$SHRD:$SHRD $SHRD/metpred3.simg python -u scripts/optimisation/optimise.py ${DATA} ${SPLIT} ${FP} ${TASK} ${M1} ${M2} ${MR} $at_once $n_times ${WS_MIN} ${HI_GAMMA} ${HI_METRIC} ${HI_MIN} $unimp_miu ${UNIMP_METRIC} $unimp_max $seed ${RESULTS_DIR} optimisation_results/exp3_filter_unimportant/s1_optimise --skip_criterion_check
          singularity run --pwd $HOME/projects/metstab_pred -B $HOME:$HOME,$SHRD:$SHRD $SHRD/metpred3.simg python -u scripts/optimisation/optimise.py ${DATA} ${SPLIT} ${FP} ${TASK} ${M1} ${M2} ${MR} $at_once $n_times ${WS_MIN} ${HI_GAMMA} ${HI_METRIC} ${HI_MIN} $unimp_miu ${UNIMP_METRIC} $unimp_max $seed ${RESULTS_DIR} optimisation_results/exp3_filter_unimportant/s1_baseline --baseline --skip_criterion_check
        done
      done
    done
  done
